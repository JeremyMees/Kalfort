...
const { pending, data: data } = useFetch('/api/v1/players/get_with_uid', {
  method: 'POST',
  body: { uid: route.params.id },
})
</script>

<template>
  <div v-if="pending" class="title-normal absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">Loading ...</div>
  <div v-else-if="!pending && data" class="flex flex-col gap-4 p-4 shadow-gray bg-slate-800 rounded relative">
    ...
    <div class="flex gap-4 items-center">
      <div class="h-[8rem] w-[8rem] min-h-[8rem] min-w-[8rem] rounded-xl overflow-hidden">
        <img :src="data.player.image" :alt="data.player.name" class="w-full h-full object-cover" />
      </div>
      <p class="title-normal">{{ data.player.name }}</p>
    </div>
    <div
  ...
  <div v-else class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex flex-col gap-4">
    <span class="title-normal">The player that you are trying to search doesn't exist</span>
    <NuxtLink to="/players">
